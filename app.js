...
const AUTH_SESSION_KEY = "scheduleDesk.authSession.v1";
const LEGACY_MIGRATION_KEY = "scheduleDesk.legacyMigrated.v1";
...
const CLOUD_TABLE = "schedule_desk_user_state";
const CLOUD_SAVE_DELAY = 700;

const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
const weekdayNames = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];

const legacyLocalPayload = {
  items: loadCollection(ITEM_KEY).map(normalizeItem).filter(Boolean),
  goals: loadCollection(GOAL_KEY).map(normalizeGoal).filter(Boolean),
  archivedTags: loadCollection(ARCHIVED_TAG_KEY).map(normalizeTag).filter(Boolean)
};
...
function initialize() {
  ...
  els.authForm.addEventListener("submit", signInFromForm);
  els.signUpButton.addEventListener("click", signUpFromForm);
  ...
  renderSignedOutState("Checking account...");
  void initializeAuth();
}
...
function persistRecords() {
  ...
  saveCollection(getUserStorageKey(ITEM_KEY), items);
  saveCollection(getUserStorageKey(GOAL_KEY), goals);
  saveCollection(getUserStorageKey(ARCHIVED_TAG_KEY), archivedTags);
...
function loadUserCachedPayload(userId) {
  return {
    items: loadCollection(`${ITEM_KEY}.${userId}`).map(normalizeItem).filter(Boolean),
    goals: loadCollection(`${GOAL_KEY}.${userId}`).map(normalizeGoal).filter(Boolean),
    archivedTags: loadCollection(`${ARCHIVED_TAG_KEY}.${userId}`).map(normalizeTag).filter(Boolean)
  };
}
...
function loadAuthSession() {
  try {
    ...
    console.warn("Could not load saved sign-in session.", error);
    return null;
  }
}
...
async function initializeAuth() {
  ...
  try {
    const session = await refreshAuthSession(savedSession.refresh_token);
    await enterAuthenticatedSession(session, "Signed in.");
  } catch (error) {
    console.warn("Saved sign-in session expired.", error);
    clearAuthSession();
    renderSignedOutState("Please sign in again.");
  }
}
...
async function signInFromForm(event) {
  ...
  try {
    const session = await requestAuthSession("token?grant_type=password", {
...
async function signUpFromForm() {
  ...
  try {
    const session = await requestAuthSession("signup", {
      email: els.authEmail.value.trim(),
      password: els.authPassword.value
    });
    if (session.access_token) {
      await enterAuthenticatedSession(session, "Account created.");
      return;
    }
...
async function signOut() {
  ...
  if (currentSession?.access_token) {
    await authFetch("logout", { method: "POST" }, currentSession.access_token).catch(() => {});
  }
  ...
  lastCloudPayloadJson = "";
  clearAuthSession();
  renderSignedOutState("Signed out.");
}

async function enterAuthenticatedSession(session, message) {
  currentSession = session;
  currentUser = session.user;
  saveAuthSession(session);

  const initial = getInitialPayloadForUser(currentUser.id);
  setActivePayload(initial.payload);
  renderSignedInShell(message);
  await syncFromCloud(initial.fromLegacy);
}

function getInitialPayloadForUser(userId) {
  const cachedPayload = loadUserCachedPayload(userId);
  if (!isRecordsPayloadEmpty(cachedPayload)) {
    return { payload: cachedPayload, fromLegacy: false };
  }

  const legacyWasMigrated = window.localStorage.getItem(LEGACY_MIGRATION_KEY) === "1";
  if (!legacyWasMigrated && !isRecordsPayloadEmpty(legacyLocalPayload)) {
    return { payload: legacyLocalPayload, fromLegacy: true };
  }

  return {
    payload: { items: [], goals: [], archivedTags: [] },
    fromLegacy: false
  };
}
...
function setActivePayload(payload) {
  ...
  goals = payload.goals.map(normalizeGoal).filter(Boolean);
  archivedTags = normalizeTagList(payload.archivedTags || []);
}

function renderSignedOutState(message = "Sign in to open your schedule.") {
...
function renderSignedInShell(message = "Signed in.") {
  els.authPanel.classList.add("hidden");
  els.workspace.classList.remove("hidden");
  els.logoutButton.classList.remove("hidden");
  els.exportDataButton.disabled = false;
  els.importDataButton.disabled = false;
  els.authStatus.textContent = currentUser.email || "Signed in";
  setAuthMessage(message);
}
...
function formatAuthError(error) {
  return error?.message || "Could not complete this sign-in request.";
}

async function requestAuthSession(path, body) {
  const session = await authFetch(path, {
    method: "POST",
...
async function syncFromCloud(fromLegacy = false) {
...
async function loadCloudRecords() {
  const url = `${SUPABASE_URL}/rest/v1/${CLOUD_TABLE}?user_id=eq.${encodeURIComponent(currentUser.id)}&select=payload`;
...
async function saveCloudRecords(payload = createRecordsPayload()) {
  const payloadJson = JSON.stringify(payload);
  const response = await cloudFetch(`${SUPABASE_URL}/rest/v1/${CLOUD_TABLE}?on_conflict=user_id`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
async function cloudFetch(url, options = {}, retry = true) {
  ...
  currentSession = await refreshAuthSession(currentSession.refresh_token);
